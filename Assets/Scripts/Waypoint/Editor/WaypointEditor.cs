    ...
    private void OnSceneGUI()
    {
        ...
        for (int i = 0; i < Waypoint.Points.Length; i++)
        {
            ...
            Vector3 newWaypointPoint = Handles.FreeMoveHandle(
                currentWaypointPoint, 0.7f, new Vector3(0.3f, 0.3f, 0.3f), Handles.SphereHandleCap);

            //create handles numbers
            GUIStyle textStyle = new GUIStyle();
            textStyle.fontStyle = FontStyle.Bold;
            textStyle.fontSize = 16;
            textStyle.normal.textColor = Color.yellow;
            Vector3 textAlligment = Vector3.down * 0.35f + Vector3.right * 0.35f;
            Handles.Label(Waypoint.CurrentPositions + 
                Waypoint.Points[i] + textAlligment, $"{i + 1}", textStyle);
